...
////                .selectFrom(member)
////                //.limit(1).fetchOne(); : fetchFirst()와 같음
////                .fetchFirst();
////
...
//     }
    ...
    public void aggregation() {
        List<Tuple> result = queryFactory // Tuple : Querydsl이 제공 여러개의 타입이 있을때에 막 꺼내올 수 있는 기능 실무에선 잘 안쓰고 Dto로 꺼내 오는 방식 사용
                .select(
                        member.count(),
                        member.age.sum(),
                        member.age.avg(),
    ...
    public void group() throws Exception {
        List<Tuple> result = queryFactory
                ...
                .from(member)
                .join(member.team, team) // member에 있는 team과, team을 join 해줌
                .groupBy(team.name)
                .fetch();
    ...
    public  void join() {
        List<Member> result = queryFactory
                .selectFrom(member)
                .join(member.team, team) // .on 으로도 사용 가능
    ...
    @Test
    public void theta_join() { // 연관관게 없어도 조회 가능한 세타 조인 막 조인
